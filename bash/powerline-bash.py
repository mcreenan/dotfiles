...
def add_cwd_segment(powerline, cwd, maxdepth):
    ...
    for n in names[:-1]:
        powerline.append(Segment(powerline, ' %s ' % n, 250, 233, powerline.separator_thin, 235))
    powerline.append(Segment(powerline, ' %s ' % names[-1], 254, 233))
...
if __name__ == '__main__':
    ...
    cwd = os.getcwd()
    p.append(Segment(p, ' %s ' % os.getenv('PSHOST'), os.getenv('PSCOLOR'), 232))
    add_cwd_segment(p, cwd, 5)
    add_repo_segment(p, cwd)
    p.append(Segment(p, ' ', 255, 0))
